    ...
    ui(new Ui::options_for_graphs)
{
    ...
    new_field->field->addItem("График из точек");
    new_field->field->addItem("Непрерывный график");
...
int options_for_graphs::getGraphType()
{
    return fields[1].field->currentIndex();
}

QVector<QString> options_for_graphs::getFields()
{
    QVector<QString> result;
    for (int i = 2; i < fields.size(); i++) {
        result.push_back(fields[i].field->currentText());
    }
    return result;
}

QString options_for_graphs::getNameTable()
{
    return fields[0].field->currentText();
}
QString options_for_graphs::getNameGraph()
{
    return namegraph->text();
}
