...
CircleGraph::CircleGraph(QWidget *_Area, DrawableObject * _DrawableObject)
{
...
{ 
    ...
    _Graph->graph(0)->setLineStyle(QCPGraph::lsNone);
    _Graph->graph(0)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, 5));
    ...
    for (int i = 2; i < _DrawableObject->fields.size(); i++) {
        ...
        _Graph->graph(i - 1)->setLineStyle(QCPGraph::lsNone);
        _Graph->graph(i - 1)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, 5));
