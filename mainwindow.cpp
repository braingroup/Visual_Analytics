...
void MainWindow::draw_graph(){
    //модальное окно
    graph_options = new GraphAdapter( new GraphFabric );
    graph_options->exec();
    ...
    graph_options->BuildGraph(graph_options->getGraphType(), new_graphic.graphic, graph_options->getNameGraph(), graph_options->getFields() , graph_options->getNameTable() );
